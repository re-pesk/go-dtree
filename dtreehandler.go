...
func (handler *DTreeHandler) ReadFile(fullName string) (err error) {
	...
	handler.Content, err = ioutil.ReadFile(handler.FileName)
	return
}

func (handler *DTreeHandler) NewFile(fullName string) (err error) {
	if len(handler.Content) < 1 {
	...
	err = ioutil.WriteFile(fullName, handler.Content, 0777)
	return
}
...
func (handler *DTreeHandler) WriteFile() (err error) {
	...
	err = ioutil.WriteFile(handler.FileName, handler.Content, 0777)
	return
}
